...
<section>
  <h2 class="text-2xl font-semibold mb-4 border-b pb-2">Economic Goals</h2>
  <p class="text-base-content/80 mb-4">
    What does the target user want to achieve or experience by engaging with
    this project?
  </p>

  <form method="POST" action="?/saveEconomicGoals" use:enhance>
    <!-- Hidden input to specify which field is being saved -->
    <input type="hidden" name="field" value="goals_economic" />
    ...
    <RichTextEditor
      ...
      value={businessGoals.goals_economic ?? ""}
    />

    <div class="mt-4 flex items-center gap-4">
      <button type="submit" class="btn btn-primary">Save Economic Goals</button>
      {#if currentForm?.action === "saveEconomicGoals" && currentForm?.field === "goals_economic"}
