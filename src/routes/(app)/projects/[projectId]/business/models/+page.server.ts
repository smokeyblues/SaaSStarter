...
export const actions: Actions = {
  saveBusinessModels: async ({
    request,
    locals: { supabase, user },
    params,
  }) => {
    const actionName = "saveBusinessModels"
    ...
    const { error: upsertError } = await supabase
      .from("project_business_details")
      .upsert(
        { project_id: params.projectId, business_models: contentString },
        { onConflict: "project_id" },
      )
    if (upsertError) {
      console.error(`Error saving business models:`, upsertError)
      return fail(500, {
        action: actionName,
        field,
