...
export const actions: Actions = {
  saveDesignAesthetic: async ({
    request,
    locals: { supabase, user },
    params,
  }) => {
    const actionName = "saveDesignAesthetic"
    if (!user) return fail(401, { action: actionName, message: "Unauthorized" })
    const formData = await request.formData()
    const field = formData.get("field")?.toString()
    const content = formData.get("content")
    if (
      field !== "design_aesthetic_description" ||
      content === null ||
      content === undefined
    ) {
      return fail(400, {
      ...
      .upsert(
        {
          ...
          design_aesthetic_description: contentString,
        },
        { onConflict: "project_id" },
      )
    if (upsertError) {
      console.error(`Error saving design aesthetic:`, upsertError)
      return fail(500, {
        action: actionName,
        field,
    ...
    return {
      ...
      action: actionName,
      field,
      message: "Design Aesthetic saved.",
    }
  },
}
