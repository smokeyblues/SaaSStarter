...
<section>
  <h2 class="text-2xl font-semibold mb-4 border-b pb-2">Design Assets List</h2>
  <p class="text-base-content/80 mb-4">
    Describe the full assets list of the project.
  </p>

  <form method="POST" action="?/saveDesignAssetsList" use:enhance>
    <!-- Hidden input to specify which field is being saved -->
    <input type="hidden" name="field" value="full_assets_list_description" />
    ...
    <RichTextEditor
      name="content"
      placeholder="Describe the full assets list of the project."
      value={designDetails.full_assets_list_description ?? ""}
    />

    <div class="mt-4 flex items-center gap-4">
      <button type="submit" class="btn btn-primary"
        >Save Design Assets List</button
      >
      {#if currentForm?.action === "saveDesignAssetsList" && currentForm?.field === "full_assets_list_description"}
