...
export const actions: Actions = {
  saveTreatmentText: async ({
    ...
    const { error: upsertError } = await supabase
      .from("project_treatments")
      .upsert(
        { project_id: params.projectId, backstory_context: contentString },
        { onConflict: "project_id" },
      )
    if (upsertError) {
      console.error(`Error saving backstory_context:`, upsertError)
      return fail(500, {
        action: actionName,
        field,
